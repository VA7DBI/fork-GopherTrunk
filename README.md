...
| D-STAR            | Frame Sync + Slow Data sync, 41-byte PCH header parser (FLAG1 + RPT2 / RPT1 / UR / MY1 / MY2 + CRC-CCITT), IsGroupCall / IsEmergency / IsData accessors, IQ → GMSK bit receiver (`internal/radio/dstar/receiver`) composing FM demod + Gaussian matched filter (BT = 0.5) + Mueller-Müller clock recovery + 2-level slicer at 4800 baud to fan `dstar.BitSink` into `ControlChannel.Process` (32-bit Frame Sync detector → 328-bit PCH window → CRC-CCITT verify → Header → Ingest), repeater state machine emitting `protocol = "dstar"` grants on group transmissions. The convolutional rate-1/2 inner FEC + scrambler + interleaver the on-air PCH wraps around the information bits are documented follow-ups; the adapter works on synthesized fixtures + pre-FEC-stripped inputs today. |
