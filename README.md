...
| D-STAR            | **Header parser only — no IQ receiver yet.** Frame Sync + Slow Data sync, 41-byte PCH header parser (FLAG1 + RPT2 / RPT1 / UR / MY1 / MY2 + CRC-CCITT), IsGroupCall / IsEmergency / IsData accessors, and a repeater state machine ready to emit `protocol = "dstar"` grants — but `internal/radio/dstar/` has no `receiver/` subdirectory and no `ccdecoder` factory entry, so the chain doesn't reach live SDR input. The IQ → GMSK → PCH receiver is the follow-up that lights this protocol up end-to-end. |
