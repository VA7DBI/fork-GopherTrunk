...
// SDR replaying synthesized D-STAR DV-mode IQ (32-bit Frame Sync +
// 41-byte PCH header carrying a CQCQCQ group call) and asserts the
// production newDStarPipeline + supervisor + API + metrics chain
// recovers the lock.
//
...
// the on-air PCH wraps around the 328-bit information field are
// deliberately skipped — the integration stream carries pre-FEC bits
// straight to the Process adapter, matching the existing dstar
// package's stated "honest deferral" for FEC.
...
//     (alternating 0/1 would prime a near-match for the 0x55555555
//     Frame Sync pattern, mis-aligning the header window)
//   - `repeats` × (32-bit Frame Sync + 328-bit PCH header)
//   - 100-bit trailer for clean flush
//
// Each PCH header carries a CQCQCQ group call with the supplied
// MY1 / RPT2 / RPT1 callsigns and a valid CRC-CCITT trailer.
